...
export const auth = (userData, isSignUp) => {
	return dispatch => {
		dispatch(authStart());
		let defaultURL = "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyAvTLUe3co--_8vVrY14r9yYfYR6uq5S_M";
		if(!isSignUp){
			defaultURL = "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyAvTLUe3co--_8vVrY14r9yYfYR6uq5S_M"
		}
		axios.post(defaultURL, userData).then( result => {

			const expDate = new Date(new Date().getTime()+result.data.expiresIn*1000);
...
export const initAuthCheck = () => {
	...
	if(expDate <= new Date()){
		...
		dispatch(authSuccess({localId:localId,
			idToken:idToken}))
}}
};
