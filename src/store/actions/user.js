...
export const fetchCustomRecipe = (recipeURL) => {
	return (dispatch) => {
		...
		axios
			...
			.then((result) => {
				...
				result.data.analyzedInstructions.forEach((instructionGroup) => {
					instructionGroup.steps.forEach((instruction) => {
						...
						analyzedInstructions = analyzedInstructions.concat(
							analyzedInstruction
						);
					});
				});
			...
			.catch((error) => {
				dispatch(
					fetchCustomRecipeFailed({
						code: 400,
						message: "We're Sorry, this recipe could not be found",
					})
				);
			});
	};
...
export const fetchCustomRecipeSuccess = (recipe) => {
	return { type: actionTypes.FETCH_CUSTOM_RECIPE_SUCCESS, recipe: recipe };
};

export const fetchCustomRecipeFailed = (error) => {
	return { type: actionTypes.FETCH_CUSTOM_RECIPE_FAILED, error: error };
};
...
export const addCustomRecipe = (recipe, authToken) => {
	return (dispatch) => {
		...
		axios
			.post(
				"https://react-recipe-project-87fc0.firebaseio.com/recipes.json?auth=" +
					authToken,
				recipe
			)
			.then((result) => {
			...
			.catch((err) => {
				dispatch(addCustomRecipeFailed(err.response.data.error));
			});
	};
};
...
export const addCustomRecipeFailed = (error) => {
	return { type: actionTypes.ADD_CUSTOM_RECIPE_FAILED, error: error };
};
...
export const fetchUserRecipes = (authToken, userId) => {
	return (dispatch) => {
		...
		axios
			.get(
				"https://react-recipe-project-87fc0.firebaseio.com/recipes.json" +
					queryParams
			)
			.then((result) => {
				let recipes = [];
...
export const fetchUserRecipesSuccess = (recipes) => {
	return { type: actionTypes.FETCH_USER_RECIPES_SUCCESS, recipes: recipes };
};

export const fetchUserRecipesFailed = (error) => {
	return { type: actionTypes.FETCH_USER_RECIPES_FAILED, error: error };
};

//updating redux state after submission

export const updateRecipe = (id, recipe) => {
	return { type: actionTypes.UPDATE_RECIPE, recipe: recipe, id: id };
};
...
export const fetchUserData = (idToken, userId) => {
	return (dispatch) => {
		...
		axios
			.get(
				`https://react-recipe-project-87fc0.firebaseio.com/users/${userId}.json?auth=${idToken}`
			)
			.then((response) => {
...
export const fetchUserDataSuccess = (data) => {
	return { type: actionTypes.FETCH_USER_DATA_SUCCESS, userData: data };
};

export const fetchUserDataFailed = (error) => {
	return { type: actionTypes.FETCH_USER_DATA_FAILED, error: error };
};
...
export const updateUserDataSuccess = (data) => {
	return { type: actionTypes.UPDATE_USER_DATA_SUCCESS, userData: data };
};

export const updateUserDataFailed = (error) => {
	return { type: actionTypes.UPDATE_USER_DATA_FAILED, error: error };
};
