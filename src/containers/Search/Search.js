...
class Search extends Component {
	...
	handleFetchMoreRecipes = () => {
		if (
			this.props.recipes.length < this.state.offset + 20 ||
		...
		return (
			<Aux>
				...
				{this.props.loading ? (
					...
					<p className="text-center Green">End of SearchList</p>
				) : this.props.recipes ? (
					<Waypoint onEnter={this.handleFetchMoreRecipes} />
				) : null}
				<Modal
				...
				>
					{this.props.loading ? (
						...
						<RecipeDetails
							recipe={
								this.props.recipes[this.state.selectedIndex]
							}
...
const mapDispatchToProps = (dispatch) => {
	return {
		onFetchSearchRecipes: (recipes) =>
			dispatch(actionCreators.fetchSearchRecipes(recipes)),
		onFetchMoreSearchRecipes: (recipes, offset) =>
			dispatch(actionCreators.fetchMoreSearchRecipes(recipes, offset)),
		onFetchSearchRecipe: (id, index) =>
			dispatch(actionCreators.fetchSearchRecipe(id, index)),
		onResetSearchRecipes: () =>
			dispatch(actionCreators.resetSearchRecipes()),
	};
};
