...
error:null}
	...
	handleChangeMode = () => {
		this.setState(prevstate => {
			return{viewMode:prevstate.viewMode === "preview" ? "edit" : "preview", error:null}
		})
	}
	deleteRecipeHandler = () => {
		this.setState({loading:true, error:null})
		axios.delete(`https://react-recipe-project-87fc0.firebaseio.com/recipes/${this.state.recipe.id}.json?auth=${this.props.idToken}`)
		.then(result=>{
	...
	componentDidMount(){
		...
		this.setState({recipe:{...initRecipe, ...this.props.recipe, userId:this.props.userId}})
	}
	...
	handleFormSubmit = () => {
		...
		this.setState({loading:true, error:null})
		axios.put(`https://react-recipe-project-87fc0.firebaseio.com/recipes/${this.state.recipe.id}.json?auth=${this.props.idToken}`, this.state.recipe)
		.then(result=>{this.handleChangeMode()
			this.setState({loading:false});
	...
	handleAddIngredient = () => {
		let updatedRecipe = {...this.state.recipe};
		let updatedIngredients = this.state.recipe.ingredients.concat({amount:"",unit:"", originalName:""});
		updatedRecipe.ingredients = updatedIngredients;
		this.setState({recipe:updatedRecipe})
	}
	...
	render(){
		let content = <RecipePreview 
		unAuthenticated={this.handleUnAuthenticated}
		servings={+this.state.recipe.servings + this.state.servingsModifier} 
		...
		if(this.state.viewMode === "edit"){
			content = <RecipeEdit recipe={this.state.recipe} 
			isSaved={this.props.savedRecipe}
			deleteRecipe={this.deleteRecipeHandler}
			tagToggleSelect={(tag)=>this.tagToggleHandler(tag)}
			instructionsChange={(event)=>this.handleInstructionsChange(event)}
				ingredientsChange={(event,identifier,index)=>this.handleIngredientsChange(event,identifier,index)}
				inputChange={(event, identifier)=>this.handleInputChange(event, identifier)}
	...
	return(
			...
			{this.state.error !== null ? <ErrorAlert error={{code:'400', message:"There was trouble trying to process your request"}} /> : this.state.loading ? <Loader /> : content}
			</div>
			)
	}
...
const mapDispatchToProps = dispatch => {
	return {
		onUpdateRecipe: (recipe, id, idToken) => dispatch(actionCreators.updateRecipe(recipe, id, idToken)),
		onAddCustomRecipe:(recipe, authToken)=>dispatch(actionCreators.addCustomRecipe(recipe, authToken)),
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(RecipeDetails));