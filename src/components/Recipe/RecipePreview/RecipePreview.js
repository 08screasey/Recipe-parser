...
const recipePreview = (props) => {
	...
	var urlParts = props.recipe.sourceUrl
		.replace("http://", "")
		.replace("www.", "")
		.replace("https://", "")
		.split(/[/?#]/);
	...
	let ingredients = props.recipe.ingredients.map((ingredient) => {
		return (
			<li key={ingredient.originalName}>
				<strong>{servingsCalculator(ingredient.amount)}</strong>{" "}
	...
	return (
		<div className="RecipePreview">
			...
			<div className="row m-3 justify-content-center">
				...
				<div className="col-md-5 d-flex flex-column align-items-center justify-content-center ServingsDiv">
					<p>
						...
						<i
							className="fas fa-plus-circle mx-1 toggler Green"
							onClick={() => props.changeServings(1)}
						></i>
						<i
							className="fas fa-minus-circle toggler redButton"
							onClick={() => props.changeServings(-1)}
						></i>
					</p>
			...
			<hr />
			<h5>Ingredients:</h5>
			<ul className="RecipePreviewList">{ingredients}</ul>
			<hr />
			<h5>Instructions:</h5>
			<ol className="RecipePreviewList">{instructions}</ol>
			{props.recipe.notes && props.recipe.notes[0].length > 0 ? (
				<Aux>
					<hr />
					<h5>My Notes</h5>
			...
			<div className="d-flex justify-content-around">
				...
				{!props.authenticated ? (
					<Button btnType="green" clicked={props.unAuthenticated}>
						Sign in to Add +
					</Button>
				) : props.saved ? null : (
					<Button clicked={props.saveRecipe} btnType="green">
						Add To My Recipes
					</Button>
				)}
			</div>
